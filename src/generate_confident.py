...
def generate_conf_plot(hparam_path, x_train, y_train, X_test, y_test):
    ...
    if rna_type == 'p':
        ...
        for limit in [alpha, 1 - alpha]:
            gbr =catboost.CatBoostRegressor(loss_function=f'Quantile:alpha={limit}', **common_params)
            all_models["q %1.2f" % limit] = gbr.fit(x_train, y_train)
        gbr_ls = catboost.CatBoostRegressor(**common_params)

    else:
        raise 'RNA TYPE NOT SUPPORTED'
    ...
    def draw_plot(q_lower, q_upper, real_values, save_plot=True):
        ...
        plt.title(f'confident score for RNA{rna_type}-{accu}%')
        plt.show()
        if save_plot:
...
if __name__ == '__main__': #TODO- use correct files for both rnai rnap
    rna_type = 'p'
