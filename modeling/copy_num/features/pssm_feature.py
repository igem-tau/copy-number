...
def is_high_copy_number(copy_number: 'pd.Series[int]', type: str='p') -> 'pd.Series[int]':
    ...
    if not os.path.exists(PSSM_THRESHOLD_PATH):
        raise Exception('you need to set pssm thresholds first - run "pssn_feature.py"')
...
def calc_series_pssm_score(seq: 'pd.Series[str]', pssm=None, type: str='p') -> 'pd.Series[float]':
    if pssm is None:
