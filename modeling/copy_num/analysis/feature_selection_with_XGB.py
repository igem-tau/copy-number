...
def get_save_path(rna_type: RNA_TYPES) -> pathlib.Path:
    return pathlib.Path(
        ...
        RELATIVE_DATA_PATH,
        f'XGB_features_selection_RNA{rna_type}.joblib'
    )
...
def main():
    ...
    for X, y, rna_type in RUNS:
        ...
        for seed in tqdm(seeds):
            ...
            selected_features, validation_score = prediction_regressor(X_train, y_train, X_valid, y_valid)
            model = xgb.XGBRegressor()
        ...
        dump(pd.concat(rows, axis=1).T, get_save_path(rna_type))
